...
class DolphinInstance:
    ...
    def verify_target_address(self, target_address: int, read_size: int):
        ...
        if target_address < 0x80000000 or target_address + read_size > 0x81800000:
            raise DolphinException(
                f"{target_address:x} -> {target_address + read_size:x} is not a valid memory address. Please write between the ranges 0x80000000 and 0x81800000."
            )
    ...
    def connect_to_game(self):
        try:
            ...
            for index in range(6):
                game_id += self.dolphin_client.read_address(HEADER_ID_ADDRESS, index)
...
async def handle_check_goal_complete(ctx: KRtDLContext):
    ...
    logger.info("yippee cool you win I haven't actually made this do anything yet")
